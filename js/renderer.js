...
ZuhyoRenderer.prototype.updateProject = function(project, instant) {
    ...
    if (instant) {
        ...
        this._dispMap = newMap;
		// Sync fill states instantly too
		for (var id in newMap) {
			var g = _grp(newMap[id].cmds)[0];
			this._fillStates[id] = { reveal: 1, lastFillStr: g && g.fill ? JSON.stringify(g.fill) : '' };
		}
        this.draw();
        return;
    }
    
    // Fill state sync with "zombie" support
    for (var id in newMap) {
        var group = _grp(newMap[id].cmds)[0];
        var fillStr = group && group.fill ? JSON.stringify(group.fill) : '';
        if (!this._fillStates[id]) {
            this._fillStates[id] = { reveal: 0, lastFillStr: fillStr, zombie: false };
        } else if (this._fillStates[id].lastFillStr !== fillStr) {
            this._fillStates[id].reveal = 0;
            this._fillStates[id].lastFillStr = fillStr;
			this._fillStates[id].zombie = false;
        } else {
			this._fillStates[id].zombie = false;
		}
    }
	// Mark removed instances as zombies
	for (var id in this._fillStates) {
		if (!newMap[id]) this._fillStates[id].zombie = true;
	}
...
ZuhyoRenderer.prototype._anim = function(now) {
    ...
    // Update fill reveals & cleanup zombies
    var hasFilling = false, dt = (now - (this._animLastT || now));
    for (var fid in this._fillStates) {
        var fs = this._fillStates[fid];
        if (fs.zombie) {
            fs.reveal = Math.max(0, fs.reveal - dt / 500);
            if (fs.reveal > 0) hasFilling = true;
            else delete this._fillStates[fid];
        } else if (fs.reveal < 1) {
            fs.reveal = Math.min(1, fs.reveal + dt / 800);
            hasFilling = true;
        }
    }
    this._animLastT = now;
    ...
    this.draw();
    if (t < 1 || hasFilling) { var self = this; this._animRAF = requestAnimationFrame(function(ts) { self._anim(ts); }); }
    else { this._dispMap = this._targetMap; this._animLastT = null; this.draw(); }
};
...
ZuhyoRenderer.prototype.draw = function() {
    ...
    this._cvBg        = style.getPropertyValue('--cv-bg').trim()        || '#fdf9f0';
    this._cvInk     = style.getPropertyValue('--cv-ink').trim()     || '#1a0800';
    this._cvGrid    = style.getPropertyValue('--cv-grid').trim()    || 'rgba(100,60,10,.07)';
    this._cvAxis    = style.getPropertyValue('--cv-axis').trim()    || 'rgba(100,60,10,.22)';
    this._cvPt        = style.getPropertyValue('--cv-pt').trim()        || '#1c0d00';
    this._cvSel     = style.getPropertyValue('--cv-sel').trim()     || '#3a1800';
    this._cvLabel = style.getPropertyValue('--cv-label').trim() || '#5a3010';

    ctx.clearRect(0, 0, W, H);
    ctx.fillStyle = this._cvBg; ctx.fillRect(0, 0, W, H);
    this._drawGrid(W, H);
...
ZuhyoRenderer.prototype._drawGrid = function(W, H) {
    ...
    var gridCol    = this._cvGrid || 'rgba(100,60,10,.07)';
    var axisCol    = this._cvAxis || 'rgba(100,60,10,.22)';
    var labelCol = this._cvLabel || '#5a3010';
    ...
    if (sc >= 20) {
        // Minor grid
        var mc = gridCol;
        ctx.strokeStyle = mc; ctx.lineWidth = 0.5; ctx.setLineDash([]);
        ctx.beginPath();
        ...
        var majC = (gridCol || 'rgba(100,60,10,.07)').replace(/[\.](\d+)\)$/, function(_, n) {
            return '.' + Math.min(99, parseInt(n, 10) * 2) + ')';
        });
        ctx.strokeStyle = majC || 'rgba(100,60,10,.12)'; ctx.lineWidth = 0.8;
        ctx.beginPath();
    ...
    if (sc > 28) {
        ctx.font = '9px Space Mono, monospace'; ctx.fillStyle = labelCol || 'rgba(100,60,10,.28)';
...
ZuhyoRenderer.prototype._drawOrigin = function() {
    ...
    // Then draw zombies (dissolving fills)
    for (var sid in this._fillStates) {
        if (this._fillStates[sid].zombie && this._dispMap[sid]) {
             this._drawInst(sid, this._dispMap[sid], true);
        }
    }
...
ZuhyoRenderer.prototype._drawInst = function(instId, inst, isZombie) {
    ...
    for (var fi = 0; fi < groups.length; fi++) {
        var g = groups[fi];
        if (g.fill && g.fill.style !== 'none') this._renderFill(g, inst.pts, instId);
    }
	if (isZombie) return; // Only draw fill for zombies
    ...
    for (var ci = 0; ci < inst.cmds.length; ci++) {
        ...
        else if (cmd.type === 'text') this._drawText(cmd, inst.pts, sel);
        else if (cmd.type === 'label') this._drawLabel(cmd, inst.pts);
        else {
            // Check custom renderer
            if (window.ZuhyoAddonAPI && window.ZuhyoAddonAPI.customCommands) {
                for (var k = 0; k < window.ZuhyoAddonAPI.customCommands.length; k++) {
                    var cDef = window.ZuhyoAddonAPI.customCommands[k];
                    if (cmd.type === cDef.type && cDef.render) {
                        cDef.render.call(this, cmd, inst.pts, sel);
                        break;
                    }
                }
            }
...
ZuhyoRenderer.prototype._drawSeg = function(seg, pts, sel) {
    ...
    ctx.save();
    ctx.strokeStyle = sel ? (this._cvSel || '#3a1800') : (this._cvInk || '#1a0800');
...
/* Fill (direct drawing) */
ZuhyoRenderer.prototype._renderFill = function(group, pts, instId) {
    if (!group.fill) return;
    var ctx = this.ctx;
    var fa = group.fill.args || [];
    var path = _ord(group.lines);
    var style = group.fill.style;
    var angle = 0, spacing = 1, dens = 1, offsetX = 0, offsetY = 0;
	
	var fs = this._fillStates[instId] || { reveal: 1 };
	var rev = fs.reveal;
    ...
    if (path.length > 0) {
        ...
        ctx.closePath();
        ctx.clip();
        this._drawPattern(style, spacing, dens, offsetX, offsetY, minX, minY, maxX, maxY, angle, rev);
        ctx.restore();
    }
    ...
    for (var i = 0; i < group.circles.length; i++) {
        ...
        ctx.arc(cc.x, cc.y, rpx, 0, Math.PI * 2);
        ctx.clip();
        this._drawPattern(style, spacing, dens, offsetX, offsetY, cc.x - rpx, cc.y - rpx, cc.x + rpx, cc.y + rpx, angle, rev);
        ctx.restore();
    }
};

ZuhyoRenderer.prototype._drawPattern = function(style, spacing, dens, offsetX, offsetY, minX, minY, maxX, maxY, angle, reveal) {
    var ctx = this.ctx;
    ...
    if (style === 'dot') {
        var dots = [];
        var MAX_DOTS = 10000;
        outer: for (var sx = Math.floor(minX - pitchPx); sx <= Math.ceil(maxX + pitchPx); sx += pitchPx) {
            for (var sy = Math.floor(minY - pitchPx); sy <= Math.ceil(maxY + pitchPx); sy += pitchPx) {
                dots.push({x: sx, y: sy});
                if (dots.length >= MAX_DOTS) break outer;
            }
        }
        var showCount = Math.floor(dots.length * reveal);
        for (var k = 0; k < showCount; k++) {
            var d = dots[k];
            ctx.beginPath();
            ctx.arc(d.x + offsetX * this.scale, d.y + offsetY * this.scale, Math.max(0.8, pitchPx * 0.08), 0, Math.PI * 2);
        ...
        for (var di = 0; di < directions.length; di++) {
            ...
            var showLines = Math.floor(lines.length * reveal);
            for (var li = 0; li < showLines; li++) {
                var s = lines[li];
