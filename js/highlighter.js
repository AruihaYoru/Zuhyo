...
function _hlCode(raw) {
    ...
    // 3.5. Repeat: repeat: N from dst [ang] = name
    if (/^repeat\s*:/i.test(trimmed)) {
        var temp = e.replace(/(repeat)/i, '<span class="hl-kw">$1</span>')
                    .replace(/(\[.*?\])/, '<span class="hl-ctrl">$1</span>');
        return _hlSymbols(temp);
    }

    // 3.6. Intersect: intersect: p1 p2 p3 p4 = name
    if (/^intersect\s*:/i.test(trimmed)) {
        var temp = e.replace(/(intersect)/i, '<span class="hl-kw">$1</span>');
        return _hlSymbols(temp);
    }
...
function _hlSymbols(html) {
    ...
    for (var i = 0; i < parts.length; i++) {
        if (parts[i] && parts[i][0] !== '<') {
            parts[i] = parts[i].replace(/\b([a-zA-Z0-9_]+)\b(?!;)/g, '<span class="hl-pt">$1</span>');
        }
    }
    return parts.join('');
}
